...
export function adherenceFrom(days: DayRecord[], plan: WeeklyPlan): {
  ...
  const answered = days.filter((d) => d.followed.length + d.skipped.length > 0);
  if (answered.length === 0) return { adherence: null, answered: 0 };

  const followed = answered.filter((d) => accepted.every((c) => d.followed.includes(c.id)));

  return { adherence: followed.length / answered.length, answered: answered.length };
}
