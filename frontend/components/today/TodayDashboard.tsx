...
export default function TodayDashboard({ locale }: { locale: string }) {
  ...
  const progression = useMemo(() => calculateProgression(state), [state]);
  ...
  return (
    <div className="game-shell" data-game-theme={state.preferences.theme}>
      ...
      {tab === 'progress' && <section className="space-y-6"><header><p className="t-eyebrow">{c.progressEye}</p><h1 className="t-h1 mt-2">{c.progressTitle}</h1><p className="t-lead mt-3">{c.progressBody}</p></header>{!state.preferences.enabled ? <div className="game-soft-card"><p>{c.progressOff}</p></div> : <><div className="game-level"><div><p className="t-eyebrow">Personal level</p><p className="t-num text-4xl mt-2">{progression.level}</p><p className="text-sm text-muted mt-1">{progression.current} action credits earned</p></div><div className="flex-1"><div className="game-quest-bar !bg-black/10"><span style={{ width: `${Math.min(100, progression.current / progression.next * 100)}%` }} /></div><p className="text-xs text-muted mt-2">Cosmetic progress only. Weight and calories never count.</p></div></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{achievementIds.map((id, index) => { const earned = state.achievements.find((a) => a.id === id); const copy = c.achievements[index]; return <article key={id} className={`game-achievement ${earned ? '' : 'opacity-45'}`}><Award size={24} /><h2 className="font-bold mt-4">{copy[0]}</h2><p className="text-sm text-muted mt-1">{copy[1]}</p><p className="text-xs mt-4 font-semibold">{earned ? `${c.earned} ${earned.earnedAt.slice(0, 10)}` : c.available}</p></article>; })}</div></>}</section>}
      ...
      {tab === 'settings' && <section className="space-y-6"><header><p className="t-eyebrow">{c.control}</p><h1 className="t-h1 mt-2">{c.settings}</h1></header><div className="game-settings">
        ...
        <div className="p-5 border-b border-line"><h2 className="font-bold">Theme</h2><p className="text-sm text-muted mt-1">New visual styles unlock through controllable actions only.</p><div className="segment mt-4">{(['mint', 'violet', 'sunrise'] as const).map((theme) => <button key={theme} disabled={!progression.unlockedThemes.includes(theme)} data-active={state.preferences.theme === theme} onClick={() => update({ ...state, preferences: { ...state.preferences, theme } })}>{theme}</button>)}</div></div>
