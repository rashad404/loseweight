...
import { createLocalId, exportGame, importGame, initialState, loadGame, rescheduleAction, saveGame, transitionAction } from '@/lib/gamification/storage';
  ...
  return (
    <div className="game-shell" data-game-theme={state.preferences.theme}>
      ...
      {tab === 'today' && <>
        ...
        ) : today.length === 0 ? <section className="game-soft-card py-10 text-center"><h2 className="t-h2">{c.emptyTitle}</h2><p className="text-muted mt-2">{c.emptyBody}</p><Link href="/onboarding" className="btn btn-primary mt-5">{c.onboarding}</Link></section> : <section className="space-y-3" aria-labelledby="actions-heading">
      ...
      {tab === 'settings' && <section className="space-y-6"><header><p className="t-eyebrow">{c.control}</p><h1 className="t-h1 mt-2">{c.settings}</h1></header><div className="game-settings">
        ...
        <div className="p-5 border-b border-line"><h2 className="font-bold">{c.theme}</h2><p className="text-sm text-muted mt-1">{c.themeBody}</p><div className="segment mt-4">{(['mint', 'violet', 'sunrise'] as const).map((theme, index) => <button key={theme} disabled={!progression.unlockedThemes.includes(theme)} data-active={state.preferences.theme === theme} onClick={() => update({ ...state, preferences: { ...state.preferences, theme } })}>{c.themes[index]}</button>)}</div></div>
        <div className="p-5"><h2 className="font-bold">{c.planMode}</h2><p className="text-sm text-muted mt-1">{c.planModeBody}</p><div className="segment mt-4">{(['loss', 'maintenance', 'paused'] as PlanMode[]).map((m, index) => <button key={m} data-active={state.preferences.mode === m} onClick={() => setMode(m)}>{c.modes[index]}</button>)}</div></div>
        <div className="p-5 border-t border-line"><h2 className="font-bold">{c.sync}</h2><p className="text-sm text-muted mt-1">{c.syncBody}</p><div className="mt-4 flex flex-wrap gap-2"><button className="btn btn-primary" onClick={async () => { if (!localStorage.getItem('lw_token')) { setSyncStatus(c.syncMessages[0]); return; } setSyncStatus(c.syncMessages[1]); try { const result = await pushProgress(state); update(result.state); await flushGameEvents(); setSyncStatus(result.conflict ? c.syncMessages[2] : c.syncMessages[3]); } catch { setSyncStatus(c.syncMessages[4]); } }}><RefreshCw size={16} />{c.syncNow}</button><button className="btn btn-ghost" onClick={() => { const text = exportGame(state); setSyncText(text); navigator.clipboard?.writeText(text); }}><Copy size={16} />{c.copy}</button><button className="btn btn-ghost" onClick={() => { const imported = importGame(syncText); if (imported) update(imported); }}><Upload size={16} />{c.import}</button></div>{syncStatus && <p className="mt-3 text-sm text-muted" role="status">{syncStatus}</p>}<textarea className="field mt-3 min-h-24 text-xs" value={syncText} onChange={(e) => setSyncText(e.target.value)} aria-label={c.sync} /></div>
      </div></section>}
    </div>
  );
}
